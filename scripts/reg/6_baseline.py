...
baselines on per-dataset regret (R²), using LOO-fair evaluation.
...
Reported candidate
------------------
kNN_top10 — best config by LOO Mean_Regret (see recommender_reg_summary).
...
# Best config selected by LOO Mean_Regret (recommender_reg_summary_600samples.csv)
...
def main():
    ...
    # Align meta to filtered pivot
    common = [ds for ds in pivot_f.columns if ds in meta.index]
    meta_f = meta.loc[common]
    pivot_f = pivot_f[common]
    X_all  = meta_f.values
    n      = pivot_f.shape[1]
    ...
    subsets = {"full": list(range(X_all.shape[1]))}
    for k in K_VALUES:
        if k <= X_all.shape[1]:
            subsets[f"top{k}"] = ranked_idx[:k].tolist()
