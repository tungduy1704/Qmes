...
Qmes implements a two-stage meta-learning pipeline. The quantum-expensive
...
|---|---|---|---|
| Extractor | [`BaseExtractor`](api/extractors.md) | Dataset $\rightarrow$ fixed-length complexity meta-feature vector | offline **and** inference |
| Evaluator (Oracle) | [`BaseEvaluator`](api/evaluators.md) | Performance score via quantum kernel $\rightarrow$ meta-labels | **offline only** |
| Recommender | [`PairwiseRecommender`](api/recommender.md) | Meta-features $\rightarrow$ ranked circuit list | offline fit, inference predict |
...
| Circuit registry ([`Qmes.circuits`](api/circuits.md)) | `CIRCUIT_POOL`, quantum kernel-matrix computation |
