...
@author: ASUS
"""
from Qmes.Qsun.Qwave import Wavefunction
from Qmes.Qsun.Qcircuit import Qubit
from Qmes.Qsun.Qgates import *
import numpy as np
import math
...
def angle_encode(sample):
    sample = np.array(sample)
    n_qubit = len(sample)
    circuit_initial = Qubit(len(sample))
    for q in range(n_qubit):
        RY(circuit_initial, q, sample[q])
    return circuit_initial

def RY_CX_linear_encode(sample, n_layers=2):
    sample = np.array(sample)
    n_qubit = len(sample)
    circuit_initial = Qubit(n_qubit)
    for _ in range(n_layers):
        for q in range(n_qubit):
            RY(circuit_initial, q, phi=sample[q])
