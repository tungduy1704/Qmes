...
_STOCHASTIC_IDX = tuple(_PROBLEXITY_NAMES.index(nm) for nm in _STOCHASTIC_NAMES) # averaged across seeds for determinism
_K_SEEDS = 10
...
def _compute_problexity(X: np.ndarray, y: np.ndarray, k_seeds: int = _K_SEEDS) -> np.ndarray:
    ...
    for s in range(k_seeds):
        ...
        if s == 0 and list(cc.report()["complexities"].keys()) != _PX_EXPECTED_KEYS:
            raise RuntimeError(
                "Problexity metric order has changed — positional labeling is no longer valid"
            )
