"""qmatch/data/regression.py

Data loader cho tabular regression datasets.
Trả về RAW (X, y) — scaling/normalization KHÔNG làm ở đây.
Extractor và Oracle tự lo scaling theo nhu cầu riêng.

Subsample policy: mỗi dataset được cap tối đa MAX_SAMPLES=300 (random, không stratified)
ngay tại loader. Cả Extractor lẫn Oracle đều nhận data đã capped.

Sources:
    - sklearn built-ins (Diabetes, California Housing)
    - UCI ML Repository (via ucimlrepo)
    - CSV files (local, path từ QMATCH_DATA_DIR)
    - Synthetic (sklearn.datasets + custom generators)
"""
...
# UCI datasets — target_col chỉ định khi default target không phải cột cần dùng
...
def _gen_heteroscedastic(
    ...
    """y = X @ coef + noise * |x_0|  (variance phụ thuộc vào x_0)."""
    rng = np.random.RandomState(random_state)
    X = rng.randn(n_samples, n_features)
    coef = rng.randn(n_features)
...
def _gen_outlier(
    ...
    """Linear target với một phần target bị nhiễu outlier lớn."""
    rng = np.random.RandomState(random_state)
    X = rng.randn(n_samples, n_features)
    coef = rng.randn(n_features)
...
def _gen_correlated(
    ...
    """Features có correlation matrix với off-diagonal = corr."""
    rng = np.random.RandomState(random_state)
...
def load_regression_datasets() -> dict[str, tuple[np.ndarray, np.ndarray]]:
    """Load tất cả regression datasets, đã subsample về MAX_SAMPLES.
